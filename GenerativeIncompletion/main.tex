\input{GenerativeIncompletion/intro.tex}
...
\textbf{Graze}\label{graze} --- \input{GenerativeIncompletion/graze.tex}

\vspace{0.5em}
\textbf{Gradual}\label{gradual_revelation} --- \input{GenerativeIncompletion/gradual.tex}

\vspace{0.5em}
\textbf{Cognition Fracture}\label{cognition_fracture} --- \input{GenerativeIncompletion/fracture.tex}
...
genuine is forming in real time, not being retrieved from a prepared position. 
...
\input{GenerativeIncompletion/fragment_vs_blur.tex}
...
Some thoughts are already minimal. Others are inherently layered --- containing multiple simultaneous signals: 
observation, feeling, uncertainty, implication. When such a perception is compressed into a single label, information 
is lost. The expression becomes heavier, more final, and less participatory than the thought that produced it.
...
something in it. A man who interferes with what is already forming 
