...
\textbf{Relational} --- the expression reflects not only the speaker's internal state,

but the state the shared field produced in him.

A perception that arose from genuine presence is already downstream

of the other person.

Congruent expression carries that origin honestly ---

it does not import a temperature the room did not generate.

When this dimension breaks, the expression may be internally true

and relationally false simultaneously:

accurate to the man, but not to the moment he was actually inside.
...
Of the several things that have arrived --- reflex, reaction, coherence, noise ---
...
It does not act during formation. Formation happens upstream, in a resonance-based

state that produces fragments on its own. What surfaces is already complete.

Selection does not shape what forms and does not watch thoughts develop mid-process.

It acts on the arrived fragment --- simply: does this belong here, or not.

Multiple fragments may arrive simultaneously --- each genuine, each
...
Selection is not generating speech. It is removing resistance to what
has already formed. Expression is already present beneath awareness ---
partial, textured, not yet a sentence. Selection does not construct what to say. It either recognises what has already formed, or stays with the moment until something true becomes clear. It recognises what is already there and does not interfere
with its passage.

The practical consequence: if nothing is there, nothing passes. Silence
is not a failure of selection. It is selection operating honestly.
...
A genuine thought with a genuine origin can still carry pressure. Not every

real perception is resonance. Impulse can arise from something that was

actually noticed --- the origin is clean, but the thought pushes to be said.

This is the form of impulse most likely to pass undetected: it is not

constructed, it is not manufactured, it simply seeks expression before

the moment has called for it.

The check is not \textit{is this real?} It is \textit{does this need to be said?}

Resonance does not need to be. It remains true whether spoken or not.

What pushes is not always wrong --- but the pushing itself is a signal worth

catching in this window, before the release, while there is still room to

simply allow the moment to pass without it.
...
What follows release is equally governed by non-interference. A clean
expression that lands and is then immediately softened, explained, or
apologised for is not a clean expression anymore. The apology fills the
space the receiver was still inhabiting. The explanation closes what the
expression had left open. Whatever completed in the receiver
stops completing --- the space is occupied from outside before the
arrival could finish.

Say it. Then leave it where it sits. Both the expression and the silence
after it belong to the moment. Neither is yours to manage once released.
...
One condition keeps Graze clean: the expression must belong to the

shared moment. Something real that is lightly placed but has no

thread connecting it to what is actually present between the two

people lands differently than it should --- not false, but disjointed.

The receiver experiences a jump cut rather than a signal. The expression

was genuine; the anchoring was absent. \textit{What is real must also
belong here.} Graze enters the shared space, not the speaker's

internal state. The difference is not in the content. It is in

whether the perception arose from what was actually between them, or

only from within him.
